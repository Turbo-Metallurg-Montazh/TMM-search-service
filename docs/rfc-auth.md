...
* Реализовать функцию создания JWT-токена с временем жизни 10 минут, который должен содержать информацию о сервисе и о его правах. Подписать приватным ключом.
* Реализовать логику in-Memory кэширование в памяти Backend
...
#### 2. Разработка на стороне Search service, проверка токенов
##### Создание middleware/зависимости для проверки JWT в Search Service
* Создать модуль авторизации на стороне Search Service
* Написать FastAPI зависимость или Middleware для валидации токена публичным ключом
* Реализовать шаги проверки: наличие заголовка, валидности подписи, срок действия; в случае ошибок return 401
##### Разделение прав доступа
* Добавить в проверку токена валидацию scopes/права на конкретное действие
* Если у сервиса (Backend) нет нужного уровня доступа в payload токена - возвращать 403 Forbidden

#### 3. Проверка работы
##### Нагрузочное тестирование
* Провести базовое нагрузочное тестирование на эндпоинты Search Service с включенной проверкой токенов
* Посмотреть на поведение сервера и убедиться, что его процессор не зависает и не перегревается от постоянной проверки сложных цифровых подписей на каждом входящем запросе.